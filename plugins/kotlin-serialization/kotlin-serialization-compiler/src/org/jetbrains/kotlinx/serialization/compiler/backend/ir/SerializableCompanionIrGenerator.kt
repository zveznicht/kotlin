...
class SerializableCompanionIrGenerator(
    ...
    private val lazeSafeModeClassDescriptor = irClass.module.findClassAcrossModuleDependencies(
    ...
    private val lazyFunctionSymbol = compilerContext.referenceFunctions(FqName("kotlin").child(Name.identifier("lazy"))).single {
        it.descriptor.valueParameters.size == 2 && it.descriptor.valueParameters[0].type == lazeSafeModeClassDescriptor.defaultType
    }

    private val publicationEntryDescriptor = lazeSafeModeClassDescriptor.enumEntries().single { it.name == Name.identifier("PUBLICATION") }
