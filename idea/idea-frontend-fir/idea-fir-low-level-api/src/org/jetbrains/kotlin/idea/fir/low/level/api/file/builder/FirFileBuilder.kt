...
internal class FirFileBuilder(
     ...
     * [FirFile] building and recording it parenting info happens at most once per each [KtFile]
     */
    fun buildRawFirFileWithCaching(
        ktFile: KtFile,
        cache: ModuleFileCache
    ): FirFile = cache.fileCached(ktFile) {
        val file = RawFirBuilder(sessionProvider.getSession(ktFile), scopeProvider, stubMode = false).buildFirFile(ktFile)
        cache.recordParentingInfo(file)
        file
    }
    ...
    /**
     * Runs [resolve] function (which is considered to do some resolve on [firFile]) under lock for [firFile]
     */
    inline fun <R> runCustomResolveResolve(firFile: FirFile, cache: ModuleFileCache, resolve: () -> R): R =
