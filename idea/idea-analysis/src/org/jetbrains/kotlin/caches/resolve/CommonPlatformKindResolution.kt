...
class CommonPlatformKindResolution : IdePlatformKindResolution {
    ...
    override fun createLibraryInfo(project: Project, library: Library): List<LibraryInfo> {
        val isOldMetadata: Boolean = library.getFiles(OrderRootType.CLASSES)
            .any { it.extension == MetadataPackageFragment.METADATA_FILE_EXTENSION }

        return if (isOldMetadata) {
            listOf(LibraryInfo(project, library))
        } else {
            library.getFiles(OrderRootType.CLASSES).mapNotNull {
                if (!it.isMetadataKlib) return@mapNotNull null
