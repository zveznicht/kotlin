...
internal class PlainTextBuildReportWriter(
    ...
    private fun printBuildAttributes(buildAttributes: BuildAttributes) {
        printMap(p, "Rebuild reason(s):", buildAttributes.rebuildReasons)
        printMap(
            p,
            "Compiler execution strategy(s):",
            buildAttributes.executionStrategies
        )
    }
...
private fun <E : Enum<*>> printMap(p: Printer, name: String, enumMap: Map<E, Int>) {
    if (enumMap.isEmpty()) return

    if (enumMap.size == 1) {
        p.println("$name: ${enumMap.keys.single()}")
        return
    }

    p.withIndent("$name:") {
        val sortedEnumMap = enumMap.mapKeysTo(TreeMap()) { (k, _) -> k.name }
