    ...
    override fun instantiateTarget(name: String): KotlinJsTarget {
        return project.objects.newInstance(
            ...
            project.whenEvaluated {
                ...
                when {
                    isBrowserConfigured && isNodejsConfigured -> KotlinBuildStatsService.getInstance()
                        ?.report(StringMetrics.JS_TARGET_MODE, "both")
                    isBrowserConfigured -> KotlinBuildStatsService.getInstance()
                        ?.report(StringMetrics.JS_TARGET_MODE, "browser")
                    isNodejsConfigured -> KotlinBuildStatsService.getInstance()
                        ?.report(StringMetrics.JS_TARGET_MODE, "nodejs")
                    !isBrowserConfigured && !isNodejsConfigured -> KotlinBuildStatsService.getInstance()
                        ?.report(StringMetrics.JS_TARGET_MODE, "none")
                }
                Unit
            }
        }
