...
import org.jetbrains.kotlin.build.metrics.*
...
internal class GradleKotlinCompilerWork @Inject constructor(
    ...
    private val metrics = BuildMetricsReporterImpl()
    ...
    override fun run() {
        try {
            ...
            reportExecutionResultIfNeeded {
                TaskExecutionResult(buildMetrics = metrics.getMetrics(), icLogLines = icLogLines)
            }
        }
    }
    ...
    private fun compileWithDaemon(messageCollector: MessageCollector): ExitCode? {
        ...
        val connection =
            metrics.measure(BuildTime.GRADLE_DAEMON_CONNECTION) {
                try {
        ...
        try {
            metrics.measure(BuildTime.GRADLE_CLEAR_JAR_CACHE) {
    ...
    private fun nonIncrementalCompilationWithDaemon(
        ...
        metrics.reportAttribute(BuildAttribute.REBUILD_REASON, "incremental compilation is not enabled")
        ...
        return metrics.measure(BuildTime.GRADLE_KOTLIN_COMPILER) {
    ...
    private fun compileOutOfProcess(): ExitCode {
        metrics.reportAttribute(BuildAttribute.EXECUTION_STRATEGY, "out-of-process")
        val rebuildReason = "out-of-process execution strategy is non-incremental"
        metrics.reportAttribute(BuildAttribute.REBUILD_REASON, rebuildReason)
        clearLocalState(outputFiles, log, reason = rebuildReason)
    ...
    private fun compileInProcess(messageCollector: MessageCollector): ExitCode {
        metrics.reportAttribute(BuildAttribute.EXECUTION_STRATEGY, "in-process")
        val rebuildReason = "in-process execution strategy is non-incremental"
        metrics.reportAttribute(BuildAttribute.REBUILD_REASON, rebuildReason)
        clearLocalState(outputFiles, log, reason = rebuildReason)

        metrics.startMeasure(BuildTime.GRADLE_KOTLIN_COMPILER, System.nanoTime())
        ...
        return try {
            ...
            metrics.endMeasure(BuildTime.GRADLE_KOTLIN_COMPILER, System.nanoTime())
        }
    }
