...
class ResolverOptionsTest : TestCase() {
    ...
    fun testParserReturnsSingleValue() {
        ...
        val options = parser("option1 = hello")
        assertEquals(options.value("option1"), "hello")
    }

    fun testParserReturnsMultipleValue() {
        val parser = SimpleExternalDependenciesResolverOptionsParser
        val options = parser("option1 = hello option2 = 42")
        assertEquals(options.value("option1"), "hello")
    ...
    fun testParserReturnsMultipleFlags() {
        val parser = SimpleExternalDependenciesResolverOptionsParser
        val options = parser("option1 option2=false option3")
    ...
    fun testParserReturnsMixOfValuesAndFlags() {
        val parser = SimpleExternalDependenciesResolverOptionsParser
        val options = parser("option1 = hello option2 option3=world option4 option5 = false")
        assertEquals(options.value("option1"), "hello")
    ...
    fun testParserReportsClashWithConflictingOptions() {
        val parser = SimpleExternalDependenciesResolverOptionsParser
        try {
            val options = parser("option1 = hello option1 = world")
            fail("Managed to parse options despite conflicting options: $options")
        } catch (exception: Exception) {
            assertEquals(exception.message, "Conflicting values for option option1: hello and world")
        }
    }

    fun testParserDoesNotClashWithTheSameOptionTwice() {
        val parser = SimpleExternalDependenciesResolverOptionsParser
        val options = parser("option1 = hello option1 = hello")
        assertEquals(options.value("option1"), "hello")
    }
}