...
abstract class AbstractTypeApproximator(val baseContext: TypeSystemInferenceExtensionContext) {
    ...
    // null means that this input type is the result, i.e. input type not contains not-allowed kind of types
    // type <: resultType
    fun approximateToSuperType(type: KotlinTypeMarker, conf: TypeApproximatorConfiguration): KotlinTypeMarker? = with(baseContext) {
        approximateToSuperType(type, conf, -type.typeDepth())
    }

    // resultType <: type
    fun approximateToSubType(type: KotlinTypeMarker, conf: TypeApproximatorConfiguration): KotlinTypeMarker? = with(baseContext) {
    ...
    private fun approximateCapturedType(
        ...
        with(baseContext) {
            ...
            if (conf.capturedType(baseContext, type)) {
                /**
    ...
    private fun KotlinTypeMarker.defaultResult(toSuper: Boolean) = with(baseContext) {
        if (toSuper) nullableAnyType() else {
            if (this is SimpleTypeMarker && isMarkedNullable()) nullableNothingType() else nothingType()
        }
    }

    // Any? or Any!
    private fun KotlinTypeMarker.isTrivialSuper() = with(baseContext) {
        upperBoundIfFlexible().isNullableAny()
    }

    // Nothing or Nothing!
    private fun KotlinTypeMarker.isTrivialSub() = with(baseContext) {
        lowerBoundIfFlexible().isNothing()
    }
}
