...
fun test_5(a: A, in1: In1<A>, in2: In1<in A>, in3: <!CONFLICTING_PROJECTION!>In1<out A><!>) {
    in1.take(a)
    in2.take(a)
    in3.<!UNRESOLVED_REFERENCE!>take<!>(a)
}

fun test_6(a: A, out1: Out1<A>, out2: <!CONFLICTING_PROJECTION!>Out1<in A><!>, out3: Out1<out A>) {
    out1.value().foo()
