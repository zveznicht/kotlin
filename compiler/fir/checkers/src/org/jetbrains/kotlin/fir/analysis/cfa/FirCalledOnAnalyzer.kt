...
object FirCalledOnAnalyzer : FirControlFlowChecker() {
    ...
    override fun analyze(graph: ControlFlowGraph, reporter: DiagnosticReporter) {
        ...
        val requiredValueSymbols = effects.filterIsInstance<ConeCalledOnEffectDeclaration>().map {
            function.getParameterSymbol(it.lambda.parameterIndex) to function.getParameterSymbol(it.value.parameterIndex)
        }.toMap()
