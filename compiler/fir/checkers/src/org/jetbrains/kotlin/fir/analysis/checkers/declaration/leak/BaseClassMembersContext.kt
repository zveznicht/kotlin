...
//    private val highOrderFunctions
    ...
    init {
        ...
        classCfg.traverseForwardWithoutLoops(
            visitor
        )
...
internal class ForwardCfgVisitor(
    ...
    val initContextNodesMap = mutableMapOf<CFGNode<*>, InitializeContextNode>()

    private var currentAffectingNodes = Stack<InitializeContextNode>()
    ...
    private var lastQualifiedAccessContextNode: InitializeContextNode? = null
...
//                    TODO: clean up with casting
    ...
    override fun visitQualifiedAccessNode(node: QualifiedAccessNode) {
        ...
        if (node.fir.calleeReference.isMemberOfTheClass(classId)) {
            ...
            initContextNodes.add(lastQualifiedAccessContextNode!!)
        } else {
            initContextNodes.add(
                checkAndBuildNodeContext(
                    cfgNode = node,
    ...
    override fun visitVariableAssignmentNode(node: VariableAssignmentNode) {
        if (node.fir.lValue.resolvedSymbolAsProperty?.callableId?.classId == classId) {
            ...
            initContextNodes.add(assignNodeContext)
        } else {
...
// TODO: if variable
    ...
    private fun updateAffectedNodesAfterAssignmentNode(assignNodeContext: InitializeContextNode) {
