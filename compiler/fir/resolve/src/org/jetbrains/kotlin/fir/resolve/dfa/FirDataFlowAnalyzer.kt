...
//            flow.addImplication((expressionVariable notEq !isEq) implies (operandVariable typeNotEq nullableNothing))
    ...
    private fun processCalledOnContract(functionCall: FirFunctionCall) {
        val function = functionCall.toResolvedCallableSymbol()?.fir as? FirFunction<*> ?: return
        ...
        for (effect in effects) {
            if (effect is ConeCalledOnEffectDeclaration) {
                val anonymousFunction = argumentMapping[effect.lambda.parameterIndex] as? FirAnonymousFunction ?: continue
                if (!anonymousFunction.isLambda || anonymousFunction.invocationKind == null) continue
