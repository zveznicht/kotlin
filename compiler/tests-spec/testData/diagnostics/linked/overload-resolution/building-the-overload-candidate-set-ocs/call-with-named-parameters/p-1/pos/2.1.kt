...
class Case1() {
    ...
    fun test() {
        Marker().<!DEBUG_INFO_AS_CALL("fqName: libPackage.Marker.foo; typeCall: function")!>foo(y=1)<!>
        Marker().<!DEBUG_INFO_AS_CALL("fqName: libPackage.Marker.invoke; typeCall: variable&invoke")!>foo(x=1)<!>
    }
}
...
fun case4(marker : Marker?) {
    marker?.<!DEBUG_INFO_AS_CALL("fqName: libPackage.Marker.foo; typeCall: function")!>foo(y=1)<!>
    marker?.<!DEBUG_INFO_AS_CALL("fqName: tests.foo; typeCall: extension function")!>foo(x=1)<!><!OPERATOR_MODIFIER_REQUIRED!>><!><!SYNTAX!><!>
}
...
fun case5(marker : Marker?) {
    fun bar(){
        fun Marker.<!EXTENSION_SHADOWED_BY_MEMBER!>foo<!>(y:Int) = TODO()
        marker?.<!DEBUG_INFO_AS_CALL("fqName: libPackage.Marker.foo; typeCall: function")!>foo(y=1)<!>
        marker?.<!DEBUG_INFO_AS_CALL("fqName: tests.foo; typeCall: extension function")!>foo(x=1)<!>
    }
    marker?.<!DEBUG_INFO_AS_CALL("fqName: libPackage.Marker.foo; typeCall: function")!>foo(y=1)<!>
    marker?.<!DEBUG_INFO_AS_CALL("fqName: tests.foo; typeCall: extension function")!>foo(x=1)<!>
}