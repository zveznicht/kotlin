...
class ExperimentalUsageChecker(project: Project) : CallChecker {
    ...
    private fun checkNullableSafeCall(resolvedCall: ResolvedCall<*>, context: CallCheckerContext) {
        ...
        if (callElement is KtCallExpression && name.identifier == callElement.calleeExpression?.name) return
    ...
    private fun CallCheckerContext.isNullableReceiver(receiverValue: ReceiverValue?): Boolean {
        ...
        if (!receiverValue.type.isNullable()) return false

        val dataFlowValue = dataFlowValueFactory.createDataFlowValue(receiverValue.expression, receiverValue.type, resolutionContext)
