...
private fun makeKotlinType(
    ...
    val kotlinTypeArguments = arguments.mapIndexed { index, it ->
        when (it) {
            ...
            is IrStarProjection -> StarProjectionImpl((classifier.wrappedDescriptor as ClassDescriptor).typeConstructor.parameters[index])
            else -> error(it)
        }
    }
    return classifier.initialDescriptor.defaultType.replace(newArguments = kotlinTypeArguments).makeNullableAsSpecified(hasQuestionMark)
}
