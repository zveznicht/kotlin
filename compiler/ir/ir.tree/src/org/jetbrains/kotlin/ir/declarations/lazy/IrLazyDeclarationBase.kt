...
abstract class IrLazyDeclarationBase(
    ...
    override var annotations: List<IrConstructorCall> by lazyVar {
        initialDescriptor.annotations.mapNotNull(typeTranslator.constantValueGenerator::generateAnnotationConstructorCall).toMutableList()
    }
    ...
    private fun createLazyParent(): IrDeclarationParent? {
        val currentDescriptor = initialDescriptor
        ...
        return when (containingDeclaration) {
            is PackageFragmentDescriptor -> run {
                val parent = this.takeUnless { it is IrClass }?.let {
                    stubGenerator.generateOrGetFacadeClass(initialDescriptor)
