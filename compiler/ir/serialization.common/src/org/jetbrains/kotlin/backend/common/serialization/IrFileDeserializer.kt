...
abstract class IrFileDeserializer(
    ...
    private fun <T : IrFunction> T.withBodyGuard(block: T.() -> Unit) {
        ...
        fun checkAnonObjectLeak(): Boolean = !symbol.isPublicApi && retType is IrSimpleType && retType.classifier.let {
            !it.isPublicApi && it !is IrTypeParameterSymbol
        }

        try {
            deserializeBodies = oldBodiesPolicy || checkInlineBody() || checkAnonObjectLeak()
            block()
        } finally {
     ...
     * }
     * object's classifier leaked outside p/f scopes and accessible on C's level so
    ...
    private fun IrField.withInitializerGuard(isPrivateProperty: Boolean, f: IrField.() -> Unit) {
        ...
        fun checkAnonObjectLeak(): Boolean = isPrivateProperty && fieldType is IrSimpleType && fieldType.classifier.let {
            !it.isPublicApi && it !is IrTypeParameterSymbol
        }

        try {
            deserializeBodies = oldBodiesPolicy || checkAnonObjectLeak()
            f()
        } finally {
            deserializeBodies = oldBodiesPolicy
        }
