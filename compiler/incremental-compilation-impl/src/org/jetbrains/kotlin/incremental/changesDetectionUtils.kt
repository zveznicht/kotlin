...
internal fun getClasspathChanges(
    ...
    if (removedClasspath.isNotEmpty()) {
        ...
        return ChangesEither.Unknown(RebuildReason.DEP_CHANGE_REMOVED_ENTRY)
    }
    ...
    val historyFiles = when (historyFilesEither) {
        ...
        is Either.Error -> {
            ...
            return ChangesEither.Unknown(RebuildReason.DEP_CHANGE_HISTORY_IS_NOT_FOUND)
        }
    }
        ...
        for (historyFile in historyFiles) {
            val allBuilds = BuildDiffsStorage.readDiffsFromFile(historyFile, reporter = reporter)
                ?: return run {
                    ...
                    ChangesEither.Unknown(RebuildReason.DEP_CHANGE_HISTORY_CANNOT_BE_READ)
                }
            ...
            if (knownBuilds.isEmpty()) {
                ...
                return ChangesEither.Unknown(RebuildReason.DEP_CHANGE_HISTORY_NO_KNOWN_BUILDS)
            }

            for (buildDiff in newBuilds) {
                if (!buildDiff.isIncremental) {
                    reporter.report { "Non-incremental build from dependency $historyFile" }
                    return ChangesEither.Unknown(RebuildReason.DEP_CHANGE_NON_INCREMENTAL_BUILD_IN_DEP)
