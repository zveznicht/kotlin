...
internal fun getClasspathChanges(
    ...
    if (withSnapshot) {
        ...
        for ((module, jarSnapshot) in jarSnapshots) {
            ...
            val diffData = JarSnapshotDiffService.compareJarsInternal(jarSnapshot, actualJarSnapshot)
        ...
        val historyFilesEither =reporter.measure(BuildTime.IC_FIND_HISTORY_FILES) { modulesApiHistory.historyFilesForChangedFiles(modifiedClasspath)
        ...
        fun analyzeHistoryFiles(): ChangesEither {for (historyFile in historyFiles) {
            ...
            val buildDiffs =
                if (withBuildHistory) {
                    ...
                    allBuilds
                } else {val (knownBuilds, newBuilds) = allBuilds.partition { it.ts <= lastBuildTS }
